...
export const Calendar: React.FC<CalendarProps> = ({ onTimeSlotSelect }) => {
  ...
  const generateTimeSlots = () => {
    ...
    for (let hour = 9; hour <= 17; hour++) {
      for (let minute of [0, 30]) {
        ...
        slots.push({
          time,
          formattedTime: format(slotDate, 'h:mm a'), // This will show like "9:00 AM"
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg p-6">
      ...
      <div className="text-center text-gray-600 font-medium mb-6 bg-gray-50 py-2 rounded-lg">
        {format(selectedDate, 'EEEE, MMMM d, yyyy')}
      </div>

      {/* Time Slots */}
      <div className="grid grid-cols-3 gap-3">
        {generateTimeSlots().map(({ time, formattedTime, available }) => (
          <button
            key={time}
            onClick={() => {
              if (available) {
